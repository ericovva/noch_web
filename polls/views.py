    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return HttpResponse(json.dumps(response),
                                content_type="application/json")  # return HttpResponseRedirect('/')
    else:
        form = UploadFileForm()
...
def blog(request):
    ...
    return render(request, 'blog.html',
                  {"blog_posts": blog_posts})  # render_to_response('blog.html',{"blog_posts": blog_posts})
...
def get_blog_post(request, blog_post_id):
    ...
    return render(request, 'blog_post.html',
                  {"title": blog_post.title, "img": blog_post.img, "user": blog_post.user, "id": blog_post.id,
                   "text": blog_post.text,
                   "comments": blog_comments})  # render_to_response('blog_post.html', {"title":  blog_post.title, "img": blog_post.img, "user": blog_post.user ,"id": blog_post.id, "text": blog_post.text,"comments": blog_comments})
...
def change_view_unlike(request):
    if request.method == "POST":
        ...
        was_rate = Rating.objects.filter(user=request.user.id, content_type=content_object_type.id,
